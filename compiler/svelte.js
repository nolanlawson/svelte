    ...
    prop.value.properties.forEach(function (computation) {
        ...
        walkThroughTopFunctionScope(body, function (node) {
            if (isThisGetCallExpression(node) && !node.callee.property.computed) {
                validator.error("Cannot use this.get(...) \u2014 values must be passed into the function as arguments", node.start);
            }
            if (node.type === 'ThisExpression') {
                validator.error("Computed properties should be pure functions \u2014 they do not have access to the component instance and cannot use 'this'. Did you mean to put this in 'methods'?", node.start);
            }
        });
        if (params.length === 0) {
...
function helpers(validator, prop) {
    ...
    prop.value.properties.forEach(function (prop) {
        ...
        walkThroughTopFunctionScope(prop.value.body, function (node) {
            if (isThisGetCallExpression(node) && !node.callee.property.computed) {
                validator.error("Cannot use this.get(...) \u2014 values must be passed into the helper function as arguments", node.start);
            }
            if (node.type === 'ThisExpression') {
...
function validateElement(validator, node, refs, refCallees, stack, elementStack) {
    ...
    if (node.name === 'title') {
        if (node.attributes.length > 0) {
            validator.error("<title> cannot have attributes", node.attributes[0].start);
        }
        node.children.forEach(function (child) {
            if (child.type !== 'Text' && child.type !== 'MustacheTag') {
                validator.error("<title> can only contain text and {{tags}}", child.start);
            }
        });
    }
    var hasIntro;
                ...
                if (definitions) {
                    definitions.value.properties.forEach(function (prop) {
                        ...
                        if (!validator_1.used[category].has(name)) {
                            validator_1.warn("The '" + name + "' " + categories_1[category] + " is unused", prop.start);
                        }
                    });
                }
            });
...
function annotateWithScopes(expression) {
    ...
    walk(expression, {
        enter: function (node, parent) {
            ...
            else if (isReference$1(node, parent)) {
...
function fixCasing(name) {
    if (svgAttributeLookup.has(name))
        return svgAttributeLookup.get(name);
...
var AwaitBlock = /** @class */ (function (_super) {
    ...
    AwaitBlock.prototype.init = function (block, stripWhitespace, nextSibling) {
        ...
        [
            ...
            var context = block.getUniqueName(arg || '_'); // TODO can we remove the extra param from pending blocks?
            var contexts = new Map(block.contexts);
            contexts.set(arg, context);
            var contextTypes = new Map(block.contextTypes);
    ...
    Component.prototype.build = function (block, parentNode, parentNodes) {
        ...
        if (isDynamicComponent) {
            ...
            block.builders.init.addBlock(deindent(templateObject_8$2 || (templateObject_8$2 = __makeTemplateObject(["\n\t\t\t\tvar ", " = ", ";\n\n\t\t\t\tfunction ", "(", ") {\n\t\t\t\t\t", "\n\t\t\t\t\treturn {\n\t\t\t\t\t\t", "\n\t\t\t\t\t};\n\t\t\t\t}\n\n\t\t\t\tif (", ") {\n\t\t\t\t\tvar ", " = new ", "(", "(", "));\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\n\t\t\t\t", "\n\t\t\t"], ["\n\t\t\t\tvar ", " = ", ";\n\n\t\t\t\tfunction ", "(", ") {\n\t\t\t\t\t", "\n\t\t\t\t\treturn {\n\t\t\t\t\t\t", "\n\t\t\t\t\t};\n\t\t\t\t}\n\n\t\t\t\tif (", ") {\n\t\t\t\t\tvar ", " = new ", "(", "(", "));\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\n\t\t\t\t",
                "\n\t\t\t"])), switch_vars.value, snippet, switch_vars.props, params, statements.length > 0 && statements.join('\n'), componentInitProperties.join(',\n'), switch_vars.value, name, expression, switch_vars.props, params, beforecreate, eventHandlers.map(function (handler) { return deindent(templateObject_9$1 || (templateObject_9$1 = __makeTemplateObject(["\n\t\t\t\t\tfunction ", "(event) {\n\t\t\t\t\t\t", "\n\t\t\t\t\t}\n\n\t\t\t\t\tif (", ") ", ".on(\"", "\", ", ");\n\t\t\t\t"], ["\n\t\t\t\t\tfunction ", "(event) {\n\t\t\t\t\t\t", "\n\t\t\t\t\t}\n\n\t\t\t\t\tif (", ") ", ".on(\"", "\", ", ");\n\t\t\t\t"])), handler["var"], handler.body, name, name, handler.name, handler["var"]); })));
...
var Window = /** @class */ (function (_super) {
    ...
    Window.prototype.build = function (block, parentNode, parentNodes) {
        ...
        if (bindings.online) {
            ...
            block.builders.destroy.addBlock(deindent(templateObject_11$6 || (templateObject_11$6 = __makeTemplateObject(["\n\t\t\t\twindow.removeEventListener(\"online\", ", ");\n\t\t\t\twindow.removeEventListener(\"offline\", ", ");\n\t\t\t"], ["\n\t\t\t\twindow.removeEventListener(\"online\", ", ");\n\t\t\t\twindow.removeEventListener(\"offline\", ", ");\n\t\t\t"])), handlerName, handlerName));
        }
    };
    return Window;
}(Node$1));
...
function dom(parsed, source, stylesheet, options) {
    ...
    return generator.generate(result, options, {
        banner: "/* " + (filename ? filename + " " : "") + "generated by Svelte v" + "1.51.1" + " */",
